...
**Interfaces:**
- Consumes: `project::{mana_home, project_name_from_dir, resolve_project_paths, ensure_project_structure, ProjectPaths}` (Task 2), `prompts::pm_prompt` (Task 10), `pty::spawn` (Task 11), `monitor::file_watcher::{watch, FsEvent}` (Task 14), `tui::app::{App, AppMode}` (Task 19), `tui::event::{map_key_event, AppEvent}` (Task 19), `tui::graph::{build_nodes, GraphNode, role_label, status_symbol}` (Task 20), `lock::load_lock` (Task 6).
- Produces: `cli::launch_pm::build_notification(event_path: &Path, reviews_dir: &Path) -> Option<String>` (pure — decides whether a given changed path warrants a PM notification, and what text to inject), `cli::launch_pm::run(agent_cli: &str) -> anyhow::Result<()>` (drives a real ratatui terminal: renders chat+graph, forwards keystrokes to the PM's PTY stdin, drains PTY output and file-watcher events every frame; not unit tested itself, covered by Task 22's manual QA — but `build_notification` and the key-event mapping it reuses from Task 19 are unit tested).

This task must actually render a screen and read keystrokes — a version that
only mirrors PTY output to stdout without a `ratatui::Terminal` render loop
and without `crossterm::event::poll`-based keyboard input does not satisfy
the spec's TUI section (`docs/superpowers/specs/2026-08-13-mana-design.md`,
"TUI" section: chat pane + input line + `Ctrl+G` graph toggle, all driven by
user keystrokes) and must be treated as incomplete.
...
Expected: FAIL until the module is wired in Step 3.
...
Run: `cargo test cli::launch_pm::`
Expected: PASS (2 tests). `run()`, `run_event_loop()`, and `draw()` drive a real terminal and are exercised manually in Task 22 (requires a real `claude` binary and an interactive terminal) — also manually confirm at that point: typed characters appear in the input line, `Enter` sends the message to the PM and clears the input, `Ctrl+G` toggles the graph pane on and off, and `Esc`/`Ctrl+C` cleanly restores the terminal (raw mode disabled, alternate screen left) even if the PM process is still running.
...
- **Placeholder scan:** Task 18 contains one intentionally-flagged leftover-scaffold line from drafting (`check_preconditions`) — explicitly called out with the exact removal instruction rather than left ambiguous; this is not a "TBD", it's a documented cleanup step with the exact diff to make.
...
- **Pre-flight ruling (recorded before Task 1 dispatch):** the original draft of Task 21 only mirrored PTY output to a buffer and printed a final line count — it never called `ratatui::Terminal::draw`, never polled `crossterm` keyboard events, and never used `tui::graph::build_nodes` despite listing it as consumed. That's a spec gap (spec's "TUI" section requires a live chat+input pane with a `Ctrl+G` graph toggle). Ruling: rewrote Task 21's code in place with a background PTY-reader thread feeding an mpsc channel, a real ratatui render loop (chat pane, input line, graph pane on toggle), and `crossterm::event::poll`-based key handling wired to `tui::event::map_key_event`. Cost if this ruling is wrong: Task 21's implementer follows a render loop shape that doesn't match how `ratatui`/`crossterm` 0.29/0.28 actually want to be driven in some minor API detail (e.g. `Frame` import path) — cheap to correct in the task review, not a structural risk.
- **Pre-flight ruling (naming):** Task 1's `Command::Uninstall { cli: String }` variant and Task 16's dispatch `cli::uninstall::run(&cli)` — a local binding named `cli` shadowing the `cli` module — compiles fine (Rust keeps type/module and value namespaces separate), but is confusing to read. Ruling: no plan-text change needed since it's not a compile error; flagged here so the Task 16 task review treats a local rename (e.g. `cli_name`) as a style nit, not something to hold up completion over.
