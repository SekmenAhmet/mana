...
use crate::tui::event::{map_key_event, AppEvent};
use crate::tui::graph::{build_nodes, role_label, status_symbol, GraphNode};
...
pub fn run(agent_cli: &str) -> anyhow::Result<()> {
    ...
    let mut app = App::new();
    let result = run_event_loop(&mut terminal, &mut app, &mut session.writer, &pty_output, &fs_events, &paths);
...
fn draw(frame: &mut ratatui::Frame, app: &App, nodes: &[GraphNode]) {
    ...
    let chat_items: Vec<ListItem> = app.chat_lines[start..].iter().map(|l| ListItem::new(l.clone())).collect();
    ...
    frame.render_widget(List::new(chat_items).block(Block::default().borders(Borders::ALL).title("Chat")), chat_area);
    frame.render_widget(Paragraph::new(app.input.as_str()).block(Block::default().borders(Borders::ALL).title("Input")), input_area);

    if let Some(graph_area) = graph_area {
        let graph_items: Vec<ListItem> = nodes
            .iter()
            .map(|n| ListItem::new(format!("[{}] {} {} — {}", role_label(&n.role), status_symbol(&n.status), n.model, n.task_uuid)))
            .collect();
        frame.render_widget(List::new(graph_items).block(Block::default().borders(Borders::ALL).title("Graph")), graph_area);
    }
}
